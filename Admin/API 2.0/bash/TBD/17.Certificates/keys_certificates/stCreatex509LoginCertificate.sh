...
##################################################################################
#
# V1.1 Ian Percival  06-Jun-2023   Added csrfToken that must be used now unless you turn off Webservices.Admin.CsrfToken.enabled
...
#          mycookie.jar file
#          valid csrfToken - we stored this in a file called csrf.token (see stLogin.sh)
#
...
#
result=$(curl -v -i -L -b myCookie.jar -s -k -w "%{http_code}" -X POST "$url/certificates" -H "csrfToken: $CSRFTOKEN" -H "accept: application/json" -H "Referer: Ian" -H "Content-Type: multipart/mixed; boundary=demoBoundary" --data-binary @stCreateX509Certificate.multi)
...
if [[ $http_status -ne 200 ]] ; then
        echo "Create X509 Certificate failure: $http_status"
        echo $json
        exit
fi
echo $json
