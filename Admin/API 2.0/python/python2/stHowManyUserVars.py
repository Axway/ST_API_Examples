...
#
#
# V1.00 Ian Percival   20-Jul-2021
#
#
# APIs used - /myself ( ST login and logout )
#             /routes ( GET and PATCH )
#
# Usage: ./stHowManyUserVars.py  routeID
#
# Test Program used to determine what the limit is to the number of userVars that can be added to a route....
#  - answer - effectively unlimited - though massively afects GUI load time! 
#
# Outputs:
...
def stUpdateRoute(index, routeId, counter):
    ...
    path2 = '/additionalAttributes/userVars.' + str(index) 
    jsonIn = [
                    {
                      'op': 'add',
                      'path': path2,
...
if __name__ == "__main__":
    ...
    stUrl = 'https://10.129.128.224:8444/api/v2.0/'
    ...
    try:
        ...
        print 'Please provide an input argument 1 for the Composite/Template Route ID'
        quit()
    ...
    while loop == True:
        ...
        index = len(cRoute['additionalAttributes'])
        print index
        ...
        if stUpdateRoute(index, compId, APICounter):
            continue
        else:
            loop = False
