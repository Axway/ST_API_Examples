...
#
#
# V1.00 Ian Percival   22-Mar-2021
#
#
...
#
# Description:
#    This script processes ALL simple routes on a system - its purpose is to remove
#    any email address that is no longer required.
#
# Outputs:
    ...
    while keepLooping:
        ...
        try:
            ...
            for item in jsonRoutes:
                id = item.get("id")
                if 'failureEmailName' not in item:
                    continue
                if item.get('failureEmailName') == None:
                    continue

                failmail = item.get("failureEmailName")
                if 'ASPAC@citi.com' in failmail:
                    print(failmail)        
                continue

                newEmail = failmail.replace("ASPAC@citi.com,","")

                stUpdateSimpleRoute(id, apiCounter, newEmail)
    
        entry+=numberToFetchEachTime            

def stUpdateSimpleRoute(id, apiCounter, newEmail):
