        ...
        try:
            ...
            for item in jsonRoutes:
                ...
                for step in jsonSteps:
                    ...
                    direction = properties.get("mFlowTypeExp")
                    if direction == None:
                        stepIndex+=1
                        continue
                    ...
                    mtrackingId = properties.get("mTrackingidExp")  # "${extract(basename(transfer.target),'\\\\.',1)}",
                    
                    if mtrackingId == "${extract(basename(transfer.target),'\\\\.',1)}":
                        stepIndex+=1
                        continue
...
def stUpdateStep(id, stepIndex, apiCounter, profileid):
    ...
    path1 = '/steps/' + str(stepIndex) + '/customProperties/mTrackingidExp'
    ...
    jsonIn = [
                    {
                      "op": "replace",
                      "path": path1,
                      "value": "${extract(basename(transfer.target),'\\\\.',1)}"
                    }
             ]
