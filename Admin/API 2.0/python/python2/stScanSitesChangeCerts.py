...
#
#
# V1.00 Ian Percival   23-Oct-2020
#
# This script reads all transfer sites and prints out the associated certificate ID 
#  not completed - need to add the update section to overwrite cert ID...
#
# APIs used - /myself ( ST login and logout )
#             /sites ( GET and PATCH )
#
# Usage: ./stScanSitesChangeCerts.py 
#
# Outputs:
...
def stGetSites(counter):

    global referer
    global stTimeout
    global stUrl

    url = stUrl + 'sites?limit=1000&offset=0' 
    ...
    try:
        ...
        errText = 'Error trying to GET sites' + str(e.code) + ' ' + str(e.read())
        writeLog(errText, 'FAIL')
        return False

    except URLError as e:
        errText = 'Failed Connecting to GET sites: ' + str(e.reason)
        writeLog(errText, 'FAIL')
...
if __name__ == "__main__":
    ...
    logFile='/home/axway/updateSites.log'

    referer = 'PippinTheCat'
    stUrl = 'https://stback.test.citi.na.axway.cloud:8444/api/v2.0/'
    ...
    cSites = stGetSites(APICounter)   

    sites = item.get("result")
    for item in sites:
        print(item.get('clientCertificate')) 

    #stUpdateSite(APICounter)
