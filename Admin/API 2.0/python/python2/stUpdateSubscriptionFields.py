...
#
#
# V1.00 Ian Percival   11-Mar-2021
# V1.01 Ian Percival   15-Mar-2021 ExtraRouting migration tool misses encryptMode, mergemode and SitPulls
#
# APIs used - /myself ( ST login and logout )
#             /subscriptions ( GET , PATCH)
#
# Description:
#    This script processes ALL subscriptions on a system - it patches the 
#    post routing on success field to be delete.
#
# Outputs:
...
def stProcessSubscriptions(apiCounter):
    global referer
    global stTimeout
    global stUrl
    ...
    while keepLooping:
        ...
        try:
            ...
            for item in jsonSubscriptions:
                id = item.get("id")
                type = item.get("type")
                if 'postProcessingActions' not in item:
                    print 'Ignoring Subscription type: ' + str(type)
                    continue

                jsonPostProcessing  = item.get("postProcessingActions")
               

                #if jsonPostProcessing.get("ppaOnSuccessInDoDelete") == True:
                #    print 'Already changed this one'
                #    continue

                print ' Updating'
                stUpdateSubscription(id, apiCounter)
               
 
            
        entry+=numberToFetchEachTime            

def stUpdateSubscription(id, apiCounter):
 
    global referer
    global stTimeout
    global stUrl

    url = stUrl + 'subscriptions/' + str(id)
     
    path1 = '/postProcessingActions/ppaOnSuccessInDoDelete'
    path2 = '/subscriptionEncryptMode'
    path3 = '/flowAttrsMergeMode'
    path4 = '/maxParallelSitPulls'
    ...
    try:
        ...
        errText = 'Error trying to Patch Subscription: ' + str(e.code) + ' ' + str(e.read())
        writeLog(errText, 'FAIL')
        return False
...
if __name__ == "__main__":
    ...
    logFile='/home/axway/UpdateSubscriptions.log'
    ...
    stepTypeToUpdate = 'CustomStepTracking'                                   #<- Change to the tyoe of step you wish to update - 
    #-------------------------------------------------------------------------------
    ...
    stProcessSubscriptions(APICounter)   
