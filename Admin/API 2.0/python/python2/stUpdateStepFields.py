...
#
#
# V1.00 Ian Percival   02-Mar-2021
# V1.01 Ian Percival   08-Mar-2021  Fix bug in loop
#
#
...
#    Once found, the script replaces existing values for two fields within 
#    that step.  In this example - the step is a customSteptracker that 
#    sends extra events to the Sentinel product.
#    Sentinel changed address and port - so this program updates all....
#
# Outputs:
        ...
        try:
            ...
            for item in jsonRoutes:
                ...
                for step in jsonSteps:
                    ...
                    port = properties.get("mPort")
                    if port == None:
                        stepIndex+=1
                        continue
                    host = properties.get("mHostName")

                    # Update this step field
                    stUpdateStep(id, stepIndex, apiCounter)
                    stepIndex+=1
 
            
        entry+=numberToFetchEachTime            

def stUpdateStep(id, stepIndex, apiCounter):
    ...
    path1 = '/steps/' + str(stepIndex) + '/customProperties/mHostName'
    path2 = '/steps/' + str(stepIndex) + '/customProperties/mPort'
    jsonIn = [
                    {
                      'op': 'replace',
                      'path': path1,
